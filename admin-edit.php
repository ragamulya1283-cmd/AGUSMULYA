...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE mobil SET nama=?, merek_id=?, kategori_id=?, tahun=?, harga=?, deskripsi=?, gambar=? WHERE id=?");
    $stmt->execute([$nama,$merek_id,$kategori_id,$tahun,$harga,$des,$gambar_url,$id]);
...
<div class="container py-5">
    ...
    <form method="post" enctype="multipart/form-data">
        <div class="mb-3"><label>Nama</label><input name="nama" value="<?=htmlspecialchars($m['nama'])?>"
                class="form-control" required></div>
        <div class="mb-3"><label>Merek</label>
            <select name="merek_id" class="form-control">
                <?php foreach($merek as $b): ?><option value="<?=$b['id']?>"
                    <?=($b['id']==$m['merek_id']?'selected':'')?>><?=htmlspecialchars($b['nama'])?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3"><label>Kategori</label>
            <select name="kategori_id" class="form-control">
                <?php foreach($kategori as $k): ?><option value="<?=$k['id']?>"
                    <?=($k['id']==$m['kategori_id']?'selected':'')?>><?=htmlspecialchars($k['nama'])?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="mb-3"><label>Tahun</label><input name="tahun" value="<?=htmlspecialchars($m['tahun'])?>"
                class="form-control" required></div>
        <div class="mb-3"><label>Harga</label><input name="harga" value="<?=htmlspecialchars($m['harga'])?>"
                class="form-control" required></div>
        ...
        <div class="mb-3">
            ...
            <div class="form-text">Upload akan menggantikan gambar lama. Jika ingin menggunakan URL, masukkan pada field
                URL di bawah.</div>
        </div>

        <div class="mb-3"><label>URL gambar (jika tidak upload)</label><input name="gambar"
                value="<?=htmlspecialchars($m['gambar'])?>" class="form-control"></div>

        <div class="mb-3"><label>Deskripsi</label><textarea name="deskripsi"
                class="form-control"><?=htmlspecialchars($m['deskripsi'])?></textarea></div>
